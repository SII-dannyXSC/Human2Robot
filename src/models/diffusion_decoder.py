...
from src.models.transformers.transformer_blocks import *
...
class DiffusionTransformer(nn.Module):
    ...
    def __init__(
        ...
        print(f"obs dim: {obs_dim}, action_dim: {action_dim}, proprio_dim: {proprio_dim}")
    ...
    def forward(self, states, actions, sigma, uncond: Optional[bool] = False):
        context = self.forward_enc_only(states, actions, sigma, uncond)
        pred_actions = self.forward_dec_only(context, actions, sigma)
        return pred_actions

    def forward_enc_only(self, states, actions=None, sigma=None, uncond: Optional[bool] = False):
        ...
        input_seq = self.concatenate_inputs(emb_t, state_embed, proprio_embed, uncond)
    ...
    def process_state_embeddings(self, states):
        states_global = self.tok_emb(states)
        proprio_embed = None
    ...
    def concatenate_inputs(self, emb_t, state_x, proprio_x, uncond=False):
