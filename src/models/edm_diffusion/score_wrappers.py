...
from src.models.diffusion_decoder import DiffusionTransformer
...
class GCDenoiser(nn.Module):
    ...
    """
    def __init__(self, action_dim, obs_dim, goal_dim, num_tokens, goal_window_size, 
                 obs_seq_len, act_seq_len, sigma_data=1., proprio_dim=8):
        super().__init__()
    ...
    def forward_context_only(self, state, action, goal, sigma, **kwargs):
        ...
        Returns:
            The output of the forward pass.
        """
        raise NotImplementedError("Forward context only is not implemented")
        # c_skip, c_out, c_in = [append_dims(x, action.ndim) for x in self.get_scalings(sigma)]
