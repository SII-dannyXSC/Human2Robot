        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if stop_step is None and i in record_step:
                    intermidate_feature_list = []
                    # ZH method
                    for feature in result[1]:
                        fea_uncon, fea_text = feature.chunk(2)
                        intermidate_feature_list.append(
                            fea_uncon + guidance_scale * (fea_text - fea_uncon)
                        )
                    record_features.append(intermidate_feature_list)
        ...
        if stop_step is not None:
            return Pose2VideoPipelineOutput(
                videos=images, record_features=[record_features]
            )

        if not return_dict:
            return images

        return Pose2VideoPipelineOutput(videos=images, record_features=record_features)
